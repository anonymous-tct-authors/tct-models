...
try:
    from nanochat.jsonl_dataloader import create_dataloader
...
def load_model_from_checkpoint(
    ...
    Infers model config from directory name (schema_tokenizer_size).
    No config.json required.

    Args:
        checkpoint_dir: Directory containing model.pt
        checkpoint_name: Name of checkpoint file (default: model.pt)
    ...
    Expected directory structure:
        checkpoints/{schema}_{tokenizer}_{size}/
        └── model.pt
    """
...
def get_tct_tokenizer(schema: str):
    ...
    Returns:
        TCT tokenizer module with encode/decode/decode_prefix/vocab_size/pad_token methods
    """
    if schema == "kubernetes":
        import tct_kubernetes_bpe_1k
        return tct_kubernetes_bpe_1k
    elif schema == "eslintrc":
        import tct_eslintrc_bpe_500
        return tct_eslintrc_bpe_500
...
def get_utf8_merge_table_path(schema: str) -> Path:
    ...
    """
    merge_table_paths = {
        "kubernetes": "bpe-merges/kubernetes-utf8-bpe-1k-matched.json",
    ...
    return _PROJECT_ROOT / merge_table_paths[schema]
...
def evaluate_validation_set(
    ...
    if create_dataloader is None:
        raise ImportError("nanochat.jsonl_dataloader not available")
...
SCHEMA_PATHS = {
    "kubernetes": _PROJECT_ROOT / "schemas" / "kubernetes.json",
    "eslintrc": _PROJECT_ROOT / "schemas" / "eslintrc.json",
    "tsconfig": _PROJECT_ROOT / "schemas" / "tsconfig.json",
}
...
def generate_tct(
    ...
    Generation starts with BOS token (pad_token = vocab_size - 1), same as training.
    """
    ...
    # Start with BOS token (same as training)
    generated_tokens = [bos_token] + list(prompt_tokens) if prompt_tokens else [bos_token]
    ...
    # Note: decode_prefix has a known bug with certain edge cases
    try:
        text, consumed, is_complete = tokenizer.decode_prefix(tokens_to_decode)
...
def generate_utf8_unconstrained(
    ...
    Generation starts with BOS token (eos_token_id = vocab_size - 1), same as training.
    """
    device = model.get_device()

    start_time = time.perf_counter()

    # BOS token is eos_token_id = vocab_size - 1 (matches training setup)
...
Examples:
  ...
  python -m scripts.eval_generation --schema kubernetes \\
    ...
    parser.add_argument("--checkpoint_name", default="model.pt",
                        help="Checkpoint filename within directory (default: model.pt)")
    ...
    if args.eval_validation:
        ...
        if args.tct_checkpoint:
            ...
            if schema_config:
                ...
                data_dir = _PROJECT_ROOT / "data" / f"{args.schema}-tct-bpe"
        ...
        if args.utf8_checkpoint:
            ...
            if schema_config:
                ...
                data_dir = _PROJECT_ROOT / "data" / f"{args.schema}-utf8-bpe"
