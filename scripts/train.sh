...
#
# Usage:
#   bash scripts/train.sh kubernetes              # Run all kubernetes experiments (small, medium, large)
#   bash scripts/train.sh kubernetes tsconfig     # Run both schemas
#   bash scripts/train.sh kubernetes small        # Run only small models
#   bash scripts/train.sh kubernetes small-deep   # Run small-deep (must be explicit)
#   bash scripts/train.sh kubernetes tct          # Run only TCT tokenizer
#   bash scripts/train.sh kubernetes resume       # Resume from latest checkpoint
#   bash scripts/train.sh kubernetes tsconfig small medium resume  # Combine options
...
if [ -z "$SCHEMAS" ]; then
    echo "Usage: bash scripts/train.sh <schema>... [size]... [tokenizer] [resume] [options]"
    echo ""
    ...
    echo "  --no-compile        Disable torch.compile (faster startup, slower training)"
    echo "  --compile_mode=X    Compile mode: default, reduce-overhead, max-autotune"
    ...
    echo ""
    echo "Examples:"
    echo "  bash scripts/train.sh kubernetes"
    echo "  bash scripts/train.sh kubernetes tsconfig eslintrc    # Run all schemas"
    echo "  bash scripts/train.sh kubernetes small tct"
    echo "  bash scripts/train.sh kubernetes tsconfig resume"
    echo "  bash scripts/train.sh kubernetes --epochs=50          # Shorter training"
    echo "  bash scripts/train.sh kubernetes tiny mini base       # Run smaller models"
    echo "  bash scripts/train.sh kubernetes constant             # No LR decay"
    exit 1
fi
...
for size in $SIZES; do
    ...
    for SCHEMA in $SCHEMAS; do
        ...
        for tokenizer in $TOKENIZERS; do
            ...
            [ -n "$NO_COMPILE" ] && EXTRA_ARGS="$EXTRA_ARGS --use_torch_compile=False"
